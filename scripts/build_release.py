...
def main():
    ...
    if output.exists():
        raise FileExistsError(
            f"Refusing to reuse an existing release directory: {output}"
        )
        ...
        if args.javascript:
            ...
            if not npm:
                raise FileNotFoundError(
                    "npm is required for JavaScript release artifacts"
                )
            ...
            shutil.make_archive(
                str(output / "kokoro-web-production"), "zip", demo / "dist"
            )
        provenance = {
